...
#include "com_yassl_cyassl_TestCTaoCrypt.h"
#include <android/log.h>
#define  LOG_TAG    "---CyaSSL---"
#define  LOGI(...)  __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__)
#define  CIPHER_NOT_ENABLED  2
...
{
    int ret = 0;
    LOGI("Entered ctaocrypt_test");

    ((func_args*)args)->return_code = -1; /* error state */

    if (CheckCtcSettings() != 1) {
        LOGI("Build vs runtime math mismatch");
        err_sys("Build vs runtime math mismatch\n", -1234);
    }
    
    if ( (ret = md5_test()) ) {
        LOGI("MD5 test failed"); 
        err_sys("MD5      test failed!\n", ret);
    }
    else{
        LOGI("MD5 test passed!");
        printf( "MD5      test passed!\n");
    }

#ifndef NO_MD4
    if ( (ret = md4_test()) ) {
        LOGI("MD4 test failed");
        err_sys("MD4      test failed!\n", ret);
    }
    else {
        LOGI("MD4 test passed!");
        printf( "MD4      test passed!\n");
    }
#endif

    if ( (ret = sha_test()) ) {
        LOGI("SHA test failed");
        err_sys("SHA      test failed!\n", ret);
    }
    else {
        LOGI("SHA test passed");
        printf( "SHA      test passed!\n");
    }

#ifndef NO_SHA256
    if ( (ret = sha256_test()) ) {
        LOGI("SHA-256 test failed");
        err_sys("SHA-256  test failed!\n", ret);
    }
    else {
        LOGI("SHA-256 test passed");
...
#ifdef CYASSL_SHA512
    if ( (ret = sha512_test()) ) {
        LOGI("SHA-512 test failed");
        err_sys("SHA-512  test failed!\n", ret);
    }
    else {
        LOGI("SHA-512 test passed");
...
#ifdef CYASSL_RIPEMD
    if ( (ret = ripemd_test()) ) {
        LOGI("RIPEMD test failed");
        err_sys("RIPEMD   test failed!\n", ret);
    }
    else {
        LOGI("RIPEMD test passed");
...
#ifndef NO_HMAC
    if ( (ret = hmac_test()) ) {
        LOGI("HMAC test failed");
        err_sys("HMAC     test failed!\n", ret);
    }
    else {
        LOGI("HMAC test passed");
        printf( "HMAC     test passed!\n");
    }
#endif

    if ( (ret = arc4_test()) ) {
        LOGI("ARC4 test failed");
        err_sys("ARC4     test failed!\n", ret);
    }
    else {
        LOGI("ARC4 test passed");
        printf( "ARC4     test passed!\n");
    }

#ifndef NO_HC128
    if ( (ret = hc128_test()) ) {
        LOGI("HC-128 test failed");
        err_sys("HC-128   test failed!\n", ret);
    }
    else {
        LOGI("HC-128 test passed");
...
#ifndef NO_RABBIT
    if ( (ret = rabbit_test()) ) {
        LOGI("RABBIT test failed");
        err_sys("Rabbit   test failed!\n", ret);
    }
    else {
        LOGI("RABBIT test passed");
...
#ifndef NO_DES3
    if ( (ret = des_test()) ) {
        LOGI("DES test failed");
        err_sys("DES      test failed!\n", ret);
    }
    else {
        LOGI("DES test passed");
...
#ifndef NO_DES3
    if ( (ret = des3_test()) ) {
        LOGI("DES3 test failed");
        err_sys("DES3     test failed!\n", ret);
    }
    else {
        LOGI("DES3 test passed");
...
#ifndef NO_AES
    if ( (ret = aes_test()) ) {
        LOGI("AES test failed");
        err_sys("AES      test failed!\n", ret);
    }
    else {
        LOGI("AES test passed");
        printf( "AES      test passed!\n");
    }
#endif

    if ( (ret = random_test()) ) {
        LOGI("RANDOM test failed");
        err_sys("RANDOM   test failed!\n", ret);
    }
    else {
        LOGI("RANDOM test passed");
        printf( "RANDOM   test passed!\n");
    }

    if ( (ret = rsa_test()) ) {
        LOGI("RSA test failed");
        err_sys("RSA      test failed!\n", ret);
    }
    else {
        LOGI("RSA test passed");
...
#ifndef NO_DSA
    if ( (ret = dsa_test()) ) {
        LOGI("DSA test failed");
        err_sys("DSA      test failed!\n", ret);
    }
    else {
        LOGI("DSA test passed");
    ...
    if ( (ret = pwdbased_test()) ) {
        LOGI("PWDBASED test failed");
        err_sys("PWDBASED test failed!\n", ret);
    }
    else {
        LOGI("PWDBASED test passed");
...
#ifdef OPENSSL_EXTRA
    if ( (ret = openssl_test()) ) {
        LOGI("OPENSSL test failed");
        err_sys("OPENSSL  test failed!\n", ret);
    }
    else {
        LOGI("OPENSSL test passed");
...
#ifdef HAVE_ECC
    if ( (ret = ecc_test()) ) {
        LOGI("ECC test failed");
        err_sys("ECC      test failed!\n", ret);
    }
    else {
        LOGI("ECC test passed");
...
static const char* clientKey  = "/sdcard/certs/client-key.der";
//static const char* clientCert = "./certs/client-cert.der";
static const char* clientCert = "/sdcard/certs/client-cert.der";
#ifdef CYASSL_CERT_GEN
//    static const char* caKeyFile  = "./certs/ca-key.der";
    static const char* caKeyFile  = "/sdcard/certs/ca-key.der";
//    static const char* caCertFile = "./certs/ca-cert.pem";
    static const char* caCertFile = "/sdcard/certs/ca-cert.pem";
#endif
...
static const char* dhKey = "/sdcard/certs/dh1024.der";
...
static const char* dsaKey = "/sdcard/certs/dsa512.der";
...
{
    LOGI("Entered native testCtc function");

    func_args args;
    LOGI("Before ctaocrypt_test call");
    ctaocrypt_test(&args);

    LOGI("After ctaocrypt_test call");

    return args.return_code;
}

/* Begin JNI wrappers for CTaoCrypt test functions */
